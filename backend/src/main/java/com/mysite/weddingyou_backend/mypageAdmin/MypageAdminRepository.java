...
public interface MypageAdminRepository extends JpaRepository<MypageAdmin, Long> {
    ...
    boolean existsByPlannerEmail(String plannerEmail);
  	...
  	@Query(value = "SELECT * FROM \"MypageAdmin\" WHERE (user_name LIKE CONCAT('%', :search, '%') \r\n"
            ...
            + "OR user_join_date::date LIKE :search::date \r\n"
            + "OR TO_CHAR(user_join_date, 'YYYY-MM') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(user_join_date, 'MM-DD') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(user_join_date, 'YYYY') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(user_join_date, 'MM') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(user_join_date, 'DD') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR planner_name LIKE CONCAT('%', :search, '%') \r\n"
            + "OR planner_email LIKE CONCAT('%', :search, '%') \r\n"
            + "OR planner_phoneNum LIKE CONCAT('%', :search, '%') \r\n"
            + "OR planner_join_date::date LIKE :search::date \r\n"
            + "OR TO_CHAR(planner_join_date, '%Y-%m') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(planner_join_date, '%m-%d') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(planner_join_date, '%Y') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(planner_join_date, '%m') LIKE CONCAT('%', :search, '%') \r\n"
            + "OR TO_CHAR(planner_join_date, '%d') LIKE CONCAT('%', :search, '%') \r\n"
