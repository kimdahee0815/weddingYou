...
function PlannerProfileDetail() {
    ...
    useEffect(() => {
        axios
            .get(`/plannerProfile/detail`, {
                params: { plannerEmail: plannerEmail },
            })
            .then(async (res) => {
                const data = res.data;
                // console.log(data);
    ...
    const goChooseEstimate = (e) => {
        ...
        if (userMatching === null) {
            ...
            if (!userMatching.includes(addPlannerEmail)) {
                let formData = new FormData();
                formData.append("estimateId", estimateId);
                formData.append("usermatching", JSON.stringify([...userMatching, addPlannerEmail]));
                axios
                    .post(`/plannerProfile/matching/users`, formData)
                    .then((res) => {
                        alert("Matching request submitted!");
                ...
                >
                    ...
                    {sessionStorage.getItem("category") === "user" ? (
                        estimates?.length !== 0 ? (
                            ...
                            <button class="btn-colour-1 " onClick={goMatch} style={{ marginTop: "50px" }}>
                                Request Estimate
                            </button>
                        )
                    ) : null}
                                        ...
                                        {currentEstimate ? (
                                            <div className="contentcontainer-detail">
                                                <div className="contentbox-detail">
                                                    ...
                                                    {JSON.parse(currentEstimate?.weddingdate).map((date, index) => {
                                                        return (
                                                ...
                                                <div className="contentbox-detail">
                                                    ...
                                                    <div className="choosebox-detail">
                                                        <div className="result-detail">{currentEstimate?.studio}</div>
                                                    </div>
                                                </div>
                                                <div className="contentbox-detail">
                                                    ...
                                                    {JSON.parse(currentEstimate?.dress).length === 0 && (
                                                        <div className="choosebox-detail">
                                                            <div className="result-noneChoose">*No selection*</div>
                                                        </div>
                                                    )}
                                                ...
                                                <div className="contentbox-detail">
                                                    {JSON.parse(currentEstimate?.img)?.length === 0 ? (
                                                        ...
                                                        <h5 style={{ marginTop: "-20px" }}>Customer Attached Images</h5>
                                                    )}
                                                ...
                                                >
                                                    ...
                                                    <div className="choosebox-detail w-100" style={{ color: "black" }}>
